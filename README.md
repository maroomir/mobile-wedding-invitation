...
- [x] `src/data.json` 청첩장에 들어갈 정보를 수정합니다.
- [x] `index.html` 파비콘, title, og tag에 쓰일 meta tag 등을 수정합니다.
...
- [x] `.env` VITE_APP_NAVERMAPS_CLIENT_ID=client_id 입력이 필요합니다.
- [x] (선택) `src/firebase.ts` likes, 방명록을 사용한다면 firebase sdk 설정이 필요합니다.
